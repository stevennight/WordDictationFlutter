...
**日语发音特别规则**：
- text: 仅假名字符串，不含声调符号或数字。例如："かいます"、"こうにゅうします"
- tone: 使用⓪①②③④⑤⑥…，组合词用"+"连接。例如："③"、"⑥"、"①+①"
- **重要：text 和 tone 必须分开，绝对不能将声调数字放在 text 中**
  * 正确示例：{"text": "かいます", "tone": "③"}
  * 错误示例：{"text": "かいます③", "tone": ""} 或 {"text": "③", "tone": ""}
- **动词ます形规则**：声调统一在ます的"ま"上（ma⬇️su）。
  * 无论原词声调，变成ます形后，声调核均在"ま"位置。
  * 例：食べる(2) → text="たべます", tone="③"
  * 例：帰る(1) → text="かえります", tone="④"
  * 例：買う(0) → text="かいます", tone="③"
  * 例：お願いします → text="おねがいします", tone="⑥"
- 禁止标注原形读音。

**英语发音规则**：
- text: IPA 音标，如 "/həˈləʊ/" 或 "/ˈkæpɪtl/"
- tone: 空字符串（因为IPA已包含重读信息）
