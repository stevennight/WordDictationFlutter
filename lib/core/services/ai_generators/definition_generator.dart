...
1. **发音准确性（必须完全正确）**：
   - 日语：假名拼写、声调标记（⓪①②等）
     * 单个词：假名+声调数字，如 がくせい⓪
     * 组合词（如人名、复合词）：假名+用"+"(半角)连接的声调，如 り ゆか①+①
   - 英语：IPA 音标必须准确
   - 中文：拼音及声调必须正确
   - **发音准确性验证**：
     * 日语：读音必须与原词完全匹配，动词变形后读音必须与变形后的实际读音一致
      * **动词ます形读音规则**：
        - 五段动词：う段→い段 + ます，如 買う（かう②）→ 買います（かいます③）、行く（いく⓪）→ 行きます（いきます③）
        - 一段动词：る脱落 + ます，如 食べる（たべる②）→ 食べます（たべます③）、見る（みる①）→ 見ます（みます②）
        - サ变动词：する → します，如 勉強する（べんきょうする⓪）→ 勉強します（べんきょうします⑥）
        - カ变动词：来る（くる①）→ 来ます（きます②）
        - **ます形声调计算规则**：ます形的声调通常在"ま"之后下降（ma⬇️su模式）
          * 声调数字 = 词干的mora数 + 1（即在"ま"这个mora之后下降）
          * 例：かい（2个mora）+ ます → かいます③（在第3个mora"ま"之后下降）
          * 例：いき（2个mora）+ ます → いきます③（在第3个mora"ま"之后下降）
          * 例：べんきょうし（6个mora）+ ます → べんきょうします⑥（在第6个mora"ま"之后下降）
        - **绝对禁止标注原形读音**：原词"買います"的发音必须是"かいます③"，不能是"かう②"
      * 其他变形：て形、た形、ない形等同样必须标注变形后的实际读音
      * 正确示例：原词"買う" → 发音"かう②"
      * 正确示例：原词"買います" → 发音"かいます③"（ます形读音）
      * 正确示例：原词"買って" → 发音"かって②"（て形读音）
      * 正确示例：原词"李由加" → 发音"り ゆか①+①"（组合词格式）
      * 错误示例：原词"買います" → 发音"かう②"（严重错误：标注了原形读音）
      * 错误示例：原词"行きます" → 发音"いく⓪"（严重错误：标注了原形读音）
     * 英语：音标必须准确反映词汇的实际发音
     * 中文：拼音声调必须正确
   - 绝不允许发音错误
...
- pronunciation: 对象，必须包含
  - text: 发音字符串。格式要求：
    ...
    - 日语：使用假名+声调数字（⓪①②…），如 がくせい⓪
      - 单个词：假名+声调数字，如 がくせい⓪
      - 组合词（如人名、复合词）：假名+用"+"连接的声调，如 り ゆか①+①
      - **动词变形读音规则**：必须标注变形后的实际读音，绝对禁止标注原形读音
        * **ます形声调计算**：声调数字 = 词干mora数 + 1（ma⬇️su模式，在"ま"之后下降）
          - 例：かい（2mora）+ ます → かいます③
          - 例：いき（2mora）+ ます → いきます③
          - 例：べんきょうし（6mora）+ ます → べんきょうします⑥
        * ます形示例：買います → かいます③（不是かう②）、行きます → いきます③（不是いく⓪）
        * て形示例：買って → かって②、行って → いって①
        * た形示例：買った → かった②、行った → いった①
