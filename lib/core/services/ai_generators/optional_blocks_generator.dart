...
2. **近义词/反义词准确性**：
   - 必须是真实的近义词/反义词
   - termHtml 是原文，可以有 ruby（如果是日语）
   - 词形、读音必须正确
   - 必须提供 termPronunciation（对象），且包含 text 字段
    - 其格式要求英语用 IPA；日语用假名+声调数字⓪①②…；中文用拼音+声调符号；其他语言用该语言通用的发音标注
   - 区别说明、例句必须准确
...
4. **Ruby标注准确性（日语）**：
   ...
   - 严禁拆词逐字标注；必须保持单词整体性，尤其是熟字训。例如：<ruby><rb>今日</rb><rt>きょう</rt></ruby> 是正确的，强行拆分为 <ruby><rb>今</rb><rt>き</rt></ruby><ruby><rb>日</rb><rt>ょう</rt></ruby> 是错误的。
...
synonyms
- 数组，0–5项，尽可能多。每项对象：
  - termHtml: 近义词词形（源语言（原文）；允许 ruby）。
  - termPronunciation: 近义词读音（对象，必须包含 text 字段）。
  - 其格式要求英语用 IPA；日语用假名+声调数字⓪①②…；中文用拼音+声调符号；其他语言用该语言通用的发音标注
  - gloss: 近义词简要含义。
...
antonyms
- 数组，0–5项，尽可能多。每项对象：
  - termHtml: 反义词词形（源语言（原文）；允许 ruby）。
  - termPronunciation: 反义词读音（对象，必须包含 text 字段）。
  - 其格式要求英语用 IPA；日语用假名+声调数字⓪①②…；中文用拼音+声调符号；其他语言用该语言通用的发音标注
  - gloss: 反义词简要含义。
