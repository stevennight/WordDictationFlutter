...
2. **近义词/反义词准确性**：
   ...
   - **近义词逻辑**：必须包含 selfHtml（原词例句）和 synHtml（近义词例句），且用词正确
   - **反义词逻辑**：必须包含 selfHtml（原词例句）和 exampleHtml（反义词例句），且用词正确

3. **Extras（补充说明）准确性**：
   - textHtml 必须完全使用目标语言（译文）
   - 如需引用原文，用「」包裹，日语原文可以在「」内加 ruby
   - 必须是真实的补充说明，比如一些人文的知识、生活化的用法、典故等。
   - **严禁泄漏指令**：内容绝不能包含"校验规则"、"JSON格式"、"必须使用..."等元数据。如果发现，必须标记为错误。
...
- 数组，0–5项，尽可能多。每项对象：
  ...
  - selfHtml: 使用原词（Prompt中的单词）的例句（源语言（原文）；允许 ruby）。
  - selfTranslation: 上述原词例句的译文。
  - **注意**：反义词部分必须提供两个例句：一个是反义词的（exampleHtml），一个是原词的（selfHtml），以便对比。

extras
- 数组，0–5项，尽可能多。每项对象：
  - textHtml: 与该词相关的简短补充说明（目标语言（译文）；允许在「…」中引述少量原文，日语原文可以在「」内带 ruby）。
  - **负面约束（禁止）**：
    - 严禁包含任何关于 JSON 格式、校验规则、辞书编辑规范、"必须使用目标语言"等元数据描述。
    - 严禁输出类似"校验通过"、"无易错点"等无意义内容。
  - 必须是真实的补充说明，比如一些人文的知识、生活化的用法、典故等。
