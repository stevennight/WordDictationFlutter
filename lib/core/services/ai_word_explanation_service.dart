...
- 日文：
  ...
  - 动词变形归一化：将各种动词变形转换为字典原形（基本形）
    * ます形 → 原形：食べます→食べる、行きます→行く、読みます→読む
    * て形 → 原形：食べて→食べる、行って→行く、読んで→読む
    * た形 → 原形：食べた→食べる、行った→行く、読んだ→読む
    * ない形 → 原形：食べない→食べる、行かない→行く、読まない→読む
  - 完整短语保持不变：包含助词（の、を、に等）或空格的短语不进行动词变形转换
  - 不要返回罗马字；不要混合假名与拉丁字符；
- 非日文：
  - promptNormalized 为原文或最常见标准写法（如大小写规范、去除多余空格、常见变体的主形）
  - 动词变形归一化：将动词变位转换为不定式/原形（如英语过去式、现在分词等）
  - 不要创造新词或合成词；''';
...
- pronunciation: 对象，可省略
  - text: 发音文本（字符串）
    * 英语：IPA 音标，如 "/həˈləʊ/"
    * 日语：仅假名字符串（不含声调数字），如 "がくせい"、"かいます"、"いって"
      - **重要：text 和 tone 必须分开，不能将声调数字放在 text 中**
      - 正确示例：text="かいます", tone="③"
      - 错误示例：text="かいます③", tone="" 或 text="③", tone=""
    * 中文：拼音（含声调符号），如 "nǐ hǎo"
    * 其他语言：该语言通用的发音标注
  - tone: 声调（字符串）
    * 日语：使用⓪①②③④⑤⑥…；组合词用"+"连接，如 "①+①"；动词变形必须标注变形后的实际声调
      - 示例：べんきょうし（6mora）+ ます → text="べんきょうします"，tone="⑥"
      - ます形：買います → text="かいます", tone="③"（不是②）、行きます → text="いきます", tone="③"（不是⓪）
      - て形：買って → text="かって", tone="②"；行って → text="いって", tone="①"
      - た形：買った → text="かった", tone="②"；行った → text="いった", tone="①"
    * 英语：由于IPA已包含重读信息，tone 必须为空字符串 ""
    * 中文：可置空或省略
