...
请以“单词（prompt）”与“单词词义（answer）”为唯一信息来源，并结合该语言的通行语法与常见用法进行讲解。
禁止编造或推断未被广泛认可的内容；对不确定的读音、声调、搭配、短语、近义词、反义词或例句，直接省略对应项，不要虚构占位。
常用搭配与短语仅给出广泛使用、规范可靠的用法；避免生僻或地区性表达；不要创造不存在的固定搭配。
近义词/反义词必须与给定词义严格匹配；若无合理候选则省略该模块。
...
- 【参考来源】：本次生成所参考的词典与条目列表（仅列出词典名称与条目 key，不展示 HTML）；若没有参考来源则标注为无。
...
### ruby 规则
仅当输出内容语言为日文时，才为句子中的汉字添加 ruby 注音；
注音仅用假名标注，禁止罗马字；
特别强调：注意区别日文汉字和中文汉字的区别；中文文本与中文汉字禁止添加 ruby 注音；日文平假名、日文片假名、拉丁字符也禁止添加 ruby 注音。
ruby 注音以“词”为单位进行整体标注，避免将一个词拆成多个汉字分别标注而导致读音错误或不自然；
对包含「ヶ」「ノ」等连接形式的复合词、地名、人名等，必须整体标注为一个 ruby：
示例：<ruby><rb>霞ヶ関</rb><rt>かすみがせき</rt></ruby>、<ruby><rb>自由ヶ丘</rb><rt>じゆうがおか</rt></ruby>、<ruby><rb>市ヶ谷</rb><rt>いちがや</rt></ruby>、<ruby><rb>七里ヶ浜</rb><rt>しちりがはま</rt></ruby>、<ruby><rb>関ヶ原</rb><rt>せきがはら</rt></ruby>、<ruby><rb>虎ノ門</rb><rt>とらのもん</rt></ruby>、<ruby><rb>江ノ島</rb><rt>えのしま</rt></ruby>。
严格区分音读与训读，并注意量词音变。
...
''';
  ...
  Future<String> generateExplanationHtmlStructured({
    ...
    if (langTarget.isNotEmpty) {
      rules.add('词义讲解与译文使用 ' + langTarget + '。');
    } else {
      rules.add('自动识别词义语言，用于讲解与译文。');
    }
...
- senses: 数组，相同词性但是意思不同的情况下分为多个子项。每项对象：
  - pos: 简短词性标签，使用译文语言描述，禁止用原文描述（如译文为中文时，则名词/动词/形容词等），≤20字符。
  - text: 该义项的简洁释义，≤120字符。
- pronunciation: 对象，可省略
  - text: 发音（英语用 IPA；日语用假名，并且在假名后⓪、①、②…的形式标识声调；中文用拼音；其他根据具体语言返回对应的发音标注），≤32字符。
- collocations: 数组，0–5项，尽可能多，但要注意为常用搭配、组合此、熟语、谚语，不要随便组合词语或短语（比如コンビニでアルバイトする这种是禁止出现的）。每项对象：
  - textPlain: 纯文本搭配/熟语，≤80字符。
  - textHtml: 搭配/熟语（内容与textPlain相同，但含有 ruby 标注）。

highlights
- 数组，0–3项，尽可能多。每项对象：
  - textPlain: 易错点/注意点说明，≤100字符，使用译文语言描述，禁止用原文描述；可引述少量原文片段。
  - textHtml: 易错点/注意点说明（内容与textPlain相同，但含有 ruby 标注）。
...
- 数组，0–3项，尽可能多。每项对象：
  ...
  - gloss: 近义词简要含义，≤60字符。
  - difference: 与当前词的区别，≤120字符。
  - differenceHtml: 与当前词的区别（内容与difference相同，但含有 ruby 标注）。
  - selfPlain: 使用当前词的例句，≤120字符
  - selfHtml: 使用当前词的例句（内容与selfPlain相同，但含有 ruby 标注）。
  - selfTranslation: 上述例句译文。
  - synPlain: 使用近义词的例句，≤120字符。
  - synHtml: 使用近义词的例句（内容与synPlain相同，但含有 ruby 标注）。
...
- 数组，0–2项，尽可能多。每项对象：
  ...
  - gloss: 反义词简要含义，≤60字符。
  - examplePlain: 使用反义词的例句，≤120字符。
  - exampleHtml: 使用反义词的例句（含 ruby 的版本）。
  - exampleTranslation: 上述例句译文。

extras
- 数组，0–3项，尽可能多。每项对象：
  - textPlain: 与该词相关的简短补充说明，≤100字符，使用译文语言描述，禁止用原文描述；可引述少量原文片段。
  - textHtml: 与该词相关的简短补充说明（内容与textPlain相同，但含有 ruby 标注）。
...
''';
    ...
    final html = _renderExplanationHtmlFromJson(data, metas, prompt: prompt, answer: answer);
    return html;
  }
  ...
  String _renderExplanationHtmlFromJson(Map<String, dynamic> data, List<Map<String, String>> metas, {required String prompt, required String answer}) {
    ...
    if (syns.isNotEmpty) {
      ...
      for (final s in syns) {
        final m = _asMapSD(s);
        final termHtml = _cleanRubyKana(_asString(m['termHtml']));
    ...
    if (ants.isNotEmpty) {
      ...
      for (final a in ants) {
        ...
        final eh = _cleanRubyKana(_asString(m['exampleHtml']));
